...
class Launch6 extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    if (now == null){
      now = DateTime.now();
    }
    DateTime thir = now.add(Duration(minutes: 30));
    String formattedDateHr = DateFormat('hh:mm').format(thir);
    String formattedDateSec = DateFormat('ss').format(thir);
    return WillPopScope(
      onWillPop: () async => false,
      child: MaterialApp(
        debugShowCheckedModeBanner: false,
        home: Scaffold(
          appBar: AppBar(
            title: Text('LEVEL SIX', style: TextStyle(fontFamily: 'OpenSans', fontWeight: FontWeight.bold),),backgroundColor: Colors.black,
          ),
          body: Stack(
            children: [
              Padding(
              ...
              Align(
                ...
                child: Padding(
                  ...
                  child: FloatingActionButton.extended(
                    ...
                    onPressed: ()async{
                      if (DateTime.now().isAfter(thir) || DateTime.now().isAtSameMomentAs(thir)){
                        if (counter > 0){
                          showDialog(
                            context: context,
