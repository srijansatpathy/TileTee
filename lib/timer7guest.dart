...
/*class Launch7 extends StatefulWidget {
  @override
  _MyAppState createState() => _MyAppState();
}

class _MyAppState extends State<Launch7> {
  var _done = false;
  @override
  void initState() {
    super.initState();
...
class Launch7 extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    if (now == null){
      now = DateTime.now();
    }
    DateTime kd = now.add(Duration(minutes: 35));
    String formattedDateHr = DateFormat('hh:mm').format(kd);
    String formattedDateSec = DateFormat('ss').format(kd);
    return WillPopScope(
      onWillPop: () async => false,
      child: MaterialApp(
        debugShowCheckedModeBanner: false,
        home: Scaffold(
          appBar: AppBar(
            title: Text('LEVEL SEVEN', style: TextStyle(fontFamily: 'OpenSans', fontWeight: FontWeight.bold),),backgroundColor: Colors.black,
          ),
          body: Stack(
            children: [
              Padding(
              ...
              Align(
                ...
                child: Padding(
                  ...
                  child: FloatingActionButton.extended(
                    ...
                    onPressed: ()async{
                      if (DateTime.now().isAfter(kd) || DateTime.now().isAtSameMomentAs(kd)){
                        if (counter > 0){
                          showDialog(
                            context: context,
